...
ibis_patients_term <- ibis_term %>%
    select(pid, days) %>% distinct() %>% mutate(cohort = "Unicare")
...
event_list <- c("emergency", "inpatient") ## in case one or more are missing
 ...
 ibis_counts <- ibis_events %>%
    ...
    pivot_wider(names_from = event_type, values_from = count) %>% 
     rename(inpatient_admits = inpatient) %>% 
    replace(is.na(.), 0)
...
ibis_event_summary <- ibis_patients_term %>% left_join(ibis_events_pre, by = "pid") %>%
    replace(is.na(.), 0) %>%
    select(pid, emergency, inpatient_admits, inpatient_days) %>% 
    mutate(inpatient = if_else(inpatient_admits == 0, 0, 1))
...
events_summary <- rbind(ibis_event_summary, MIIA_event_summary) %>% 
